...
class MinJerkTrajectory3D(object):
...
class MinJerkTrajectoryFlat(object):

    def __init__(self, waypoints_xy, times):
        waypoints_xy = np.asarray(waypoints_xy, dtype=float)
        self.times = np.asarray(times, dtype=float)
        self.traj_x = MinJerkTrajectory1D(waypoints_xy[:, 0], times)
        self.traj_y = MinJerkTrajectory1D(waypoints_xy[:, 1], times)
    ...
    def sample(self, dt=0.05):
        t0, tf = (self.times[0], self.times[-1])
        ts = np.arange(t0, tf + 1e-09, dt)
        F = np.array([self.eval(t, 0) for t in ts])
        F_d = np.array([self.eval(t, 1) for t in ts])
        F_dd = np.array([self.eval(t, 2) for t in ts])
        F_ddd = np.array([self.eval(t, 3) for t in ts])
        return (ts, F, F_d, F_dd, F_ddd)
