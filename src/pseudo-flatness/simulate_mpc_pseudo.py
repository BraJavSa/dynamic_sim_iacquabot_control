...
from main_pseudo import solve_with_saturation_check, plot_results
...
class USV_MPC:
    ...
    def __init__(self, dt_mpc=0.2, N_mpc=35):
        ...
        Q = np.diag([25.0, 25.0, 35.0, 2.0, 2.0, 5.0])
...
def generate_animation(t_sim, x_d_array, y_d_array, psi_d_array, hist_x_9, hist_y_9, hist_psi_9, hist_x_opt, waypoints_xyz, anim_outpath=None, fps=20, t_wp7=None):
    if anim_outpath is None:
        anim_outpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'usv_cascade_mpc_pseudo_animation.gif')
    fig, ax = plt.subplots(figsize=(11, 8))
    waypoints_show = waypoints_xyz[:8]
    ax.plot(x_d_array, y_d_array, 'r--', lw=2, alpha=0.7, label='1. Trayectoria Deseada (Planeador Pseudo-Planitud)')
    ax.plot(hist_x_9, hist_y_9, 'b-', lw=2, alpha=0.8, label='2. Planta Real USV Iacquabot (Controlador MPC)')
    ax.scatter(waypoints_show[:, 0], waypoints_show[:, 1], c='red', s=60, zorder=5, label='Waypoints')
    ...
    ax.set_xlabel('X [m]')
    ax.set_ylabel('Y [m]')
    ax.set_title('Navegación en Cascada USV Iacquabot: Planeador Pseudo-Plano + Controlador MPC')
    ax.legend(loc='upper right', fontsize=8)
    ax.grid(True)
    ax.set_aspect('equal')
    max_idx = len(t_sim) if t_wp7 is None else np.searchsorted(t_sim, t_wp7)
    frame_step = max(1, int(max_idx / 150))
    frame_indices = np.arange(0, max_idx, frame_step)
...
def run_simulation(waypoints_xyz=None, times_initial=None, outpath=None, anim_outpath=None, generate_anim=True, return_data=False):
    ...
    if waypoints_xyz is None:
        waypoints_xyz = np.array([[0.0, 0.0], [8.0, 0.0], [14.0, 5.0], [14.0, 13.0], [20.0, 17.0], [28.0, 17.0], [32.0, 10.0], [26.0, 4.0], [29.0, 1.5]])
    if times_initial is None:
        times_initial = np.array([0.0, 7.0, 14.0, 20.0, 27.0, 33.0, 40.0, 48.0, 55.0])

    print('=== ETAPA 1: Planeador Offline (Minimum-Jerk con PSEUDO-Planitud, m11!=m22) ===')
    ...
    u_9, v_9, r_9 = (u_d_array[0], v_d_array[0], r_d_array[0])
    ...
    for i in range(n_steps):
        ...
        u_opt, x_opt = mpc.solve(x0_curr, X_ref_vals)
    ...
    hist_x_9_plot = hist_x_9[mask_wp7]
    hist_y_9_plot = hist_y_9[mask_wp7]
    psi_d_plot = psi_d_array[mask_wp7]
    hist_psi_9_plot = hist_psi_9[mask_wp7]
    u_d_plot = u_d_array[mask_wp7]
    hist_u_9_plot = hist_u_9[mask_wp7]
    v_d_plot = v_d_array[mask_wp7]
    hist_v_9_plot = hist_v_9[mask_wp7]
    r_d_plot = r_d_array[mask_wp7]
    hist_r_9_plot = hist_r_9[mask_wp7]
    err_pos_9_plot = err_pos_9[mask_wp7]
    err_psi_9_plot = err_psi_9[mask_wp7]
    ...
    waypoints_show = waypoints_xyz[:8]
    ax = axs[0, 0]
    ax.plot(x_d_plot, y_d_plot, 'r--', lw=2, label='Etapa 1: Planeador Pseudo-Plano (Deseada)')
    ax.plot(hist_x_9_plot, hist_y_9_plot, 'b-', lw=2, label='Etapa 2: Planta Real (MPC + T200)')
    ax.scatter(waypoints_show[:, 0], waypoints_show[:, 1], c='k', s=50, zorder=5, label='Waypoints')
    ax.set_xlabel('x [m]')
    ax.set_ylabel('y [m]')
    ax.set_title('Seguimiento de Trayectoria 2D (Cascada, Pseudo-Planitud)')
    ax.axis('equal')
    ax.legend(fontsize=8)
    ax.grid(True)
    ax = axs[0, 1]
    ax.plot(t_plot, np.degrees(psi_d_plot), 'r--', lw=2, label='$\\psi$ Deseado (Planeador) [deg]')
    ax.plot(t_plot, np.degrees(hist_psi_9_plot), 'b-', lw=2, label='$\\psi$ Real (MPC) [deg]')
    ax.set_xlabel('t [s]')
    ...
    ax.plot(t_plot, hist_u_9_plot, 'r-', label='u real')
    ax.plot(t_plot, v_d_plot, 'g--', label='v plan (drift, ya NO es 0)')
    ax.plot(t_plot, hist_v_9_plot, 'g-', label='v real (derrape)')
    ax.plot(t_plot, r_d_plot, 'b--', label='r plan')
    ax.plot(t_plot, hist_r_9_plot, 'b-', label='r real')
    ax.set_xlabel('t [s]')
    ...
    ax.grid(True)
    ax = axs[1, 1]
    ax.plot(t_plot, err_pos_9_plot, 'b-', label='Error de Posición [m]')
    ax.plot(t_plot, err_psi_9_plot, 'm-', label='Error de Rumbo Yaw [deg]')
    ax.set_xlabel('t [s]')
    ax.set_title('Errores de Seguimiento de la Planta Real con MPC')
    ax.legend(fontsize=8)
    ax.grid(True)
    ax = axs[2, 0]
    ax.plot(t_plot, tau_u_plan_plot, 'k:', label='$\\tau_u$ plan (Pseudo-Planitud)')
    ax.plot(t_plot, hist_Tu_dem_plot, 'r--', label='$\\tau_u$ MPC dem')
    ax.plot(t_plot, hist_Tu_act_plot, 'r-', label='$\\tau_u$ real T200')
    ax.plot(t_plot, tau_r_plan_plot, 'g:', label='$\\tau_r$ plan (Pseudo-Planitud)')
    ...
    plt.tight_layout()
    if outpath is None:
        outpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'usv_cascade_mpc_pseudo_simulation.png')
